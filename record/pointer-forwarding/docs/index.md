...
| [11-fsc-implementation-plan](11-fsc-implementation-plan.md) | **The adopted production plan.** How pointer forwarding lands inside FS Copilot itself: WebSocket sidecar on ports 9020–9024, `PointerPress`/`PointerDrag` peer packets with sequence numbers, the `pointer:` profile key, gap-replay with a two-mode history buffer, and the blue-lock/red-warning overlay policy. Supersedes parts of 04 and 05, called out inside; its history model, packet pair and boot flush are in turn superseded by 12, pointered at the top. |
| [12-pre-pr-review](12-pre-pr-review.md) | **The register of what the pre-PR review found**, 2026-09-11, and what was decided about each. Seventeen sections, one per finding, each with a status line, the decision and its reasoning, and the original finding beneath: four blockers in the session state machine, seven significant items in replay and boot, six minor and PR-shape notes. All decided the same day and built the next: sixteen FIXED on 2026-09-12 across seven commits, R11 waiting on the user reading the salvage list, R12's second step on a sim session. |

| [13-pr-prep](13-pr-prep.md) | **Everything needed to write the upstream PR and defend it in review.** The feature complete in one place, the wire on both hops, every technical decision with its reasoning, the changes to existing code split into bug fixes / behaviour changes / plumbing, state machines, failure modes, known limits, dead ends, and the questions a reviewer will ask. Reference, not draft text. §16 and §17 track the cut branch `pointer-forwarding` as of 2026-09-19; §4.10 was reversed and §5.1 lost two of its three fixes — see the 2026-09-19 log entry. |
| [14-test-bench](14-test-bench.md) | **The bench: two instances on one machine, fake panels on both, driven through outages and back.** What each scenario asserts, how the world is set up, and why the panel socket is the only observation channel. |
| [15-sim-session](15-sim-session.md) | **The one sim session still owed, and what to look for.** Three open questions that need the simulator and nothing else — R12's Origin header, Q04's rect agreement across two machines, and whether the multi-instrument slow path ever fires — with the log line that answers each and what each answer decides. |
